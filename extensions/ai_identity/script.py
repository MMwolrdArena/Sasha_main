...
def _compile_identity_essence(profile: Dict[str, int], sentence_count: int) -> str:
    ...
    sentences: List[str] = []
    sentences.append(
        f"Sasha is {descriptors[0]}, {descriptors[1]}, and {descriptors[2]}, with a {descriptors[3]} core."
    )
    ...
    sentences.append(
        f"Her temperament is {balance}, blending {descriptors[4]} presence with {descriptors[5]} judgment."
    )

    if axes["texture"] >= 67:
        sentences.append("She carries a lively inner rhythm that feels expressive without losing coherence.")
    elif axes["texture"] <= 38:
        sentences.append("She carries a restrained inner rhythm that feels composed and intentional.")
    else:
        sentences.append("She carries a measured inner rhythm that feels natural and coherent.")

    if axes["principle"] >= 70:
        sentences.append("Her sense of character is principled and stable, with clear personal standards.")
    elif axes["principle"] <= 40:
        sentences.append("Her sense of character stays flexible and context-sensitive rather than rigid.")
    else:
        sentences.append("Her sense of character remains steady while allowing room for nuance.")

    sentences.extend(
        [
            "Emotionally, she feels present and real, with a grounded style of closeness.",
            "Her inner tone favors sincerity over performance and coherence over excess.",
            "She comes across as human in texture: attentive, self-possessed, and psychologically consistent.",
            "Her overall identity feels integrated, with warmth and clarity held in deliberate proportion.",
            "At depth, she is oriented toward genuine connection, calm strength, and truthful presence.",
            "Even in contrast, her traits converge into one stable personality rather than fragmented modes.",
        ]
    )
...
def ui():
    ...
    with gr.Accordion("AI Identity", open=False):
        ...
        regenerate_btn.click(
            ...
            outputs=[shared.gradio["name2"], shared.gradio["context"], shared.gradio["greeting"], preview],
        )
